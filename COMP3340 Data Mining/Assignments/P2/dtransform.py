...
def discretise_dataset(data, classes, attr_axis=1):
    # TODO passing in a pre-discretised dataset causes an infinite loop
    if attr_axis == 0:
...
def __disc_recurs__(data, class_list, inst_count, split_list):
    ...
    for i, val in enumerate(data):  # https://stackoverflow.com/questions/20441980/add-item-to-pandas-series
        if val <= best_split:
