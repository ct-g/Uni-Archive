...
def mst(V, D, save_as="graph-mst.gml", labels=[]):
    MST = nx.Graph()
    mst_labels = {}
    mst_edges =[]
    curr_vert = 0
...
def rngraph(V, D, save_as="graph-rng.gml", labels=[]):
    R = nx.Graph()
    rng_labels = {}
    rng_edges = []
...
def gg_graph(V, D, save_as="graph-gg.gml", labels=[]):
    GG = nx.Graph()
    gg_labels = {}
    gg_edges = []
...
def knn(V, D, k, save_as="graph-knn.gml", labels=[]):
    KNN = nx.Graph()
    knn_vertices = set()
    knn_labels = {}
    knn_edges = []
...
def mst_knn(mst_edges, knn_edges, saveas="graph-mst-knn.gml"):
